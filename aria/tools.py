...
def free_web_search(query: str, max_results: int = 5) -> list[Evidence]:
    try:
        return asyncio.run(async_free_web_search(query, max_results=max_results))
    except (ImportError, RuntimeError):
        return threaded_free_web_search(query, max_results=max_results)


def threaded_free_web_search(query: str, max_results: int = 5) -> list[Evidence]:
...
async def async_free_web_search(query: str, max_results: int = 5) -> list[Evidence]:
    try:
        ...
        raise ImportError("aiohttp is not installed") from exc
    ...
    for result in results:
        if isinstance(result, Exception):
            evidence.append(
                Evidence(
                    title="Async search unavailable",
                    summary=f"Async retrieval failed: {type(result).__name__}.",
                    source_type="system",
                    score=0.0,
